SELECT 
sum(if(items LIKE '%DHB%',1,0)) as DHB
,sum(if(items LIKE '%OPV%',1,0)) as OPV
,sum(if(items LIKE '%IPV%',1,0)) as IPV
,sum(if(items LIKE '%MMR%',1,0)) as MMR
,sum(if(items LIKE '%DTP%',1,0)) as DTP
,sum(if(items LIKE '%LAJE%',1,0)) as LAJE
FROM 
(
