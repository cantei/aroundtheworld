...
?>
<?php 
$query = $conn->prepare("select be from me_tb_reg group by be  order by be desc limit 5"); 
$query->execute();
$arr_option = $query->fetchAll(PDO::FETCH_COLUMN,0);
$json_list=json_encode($arr_option);
?>
...
if (isset($_POST['checkbox']) AND $_POST['checkbox'] != '')
{
	...
	$stmt = $conn->prepare("SELECT t3.HOSPCODE,t1.be,t1.tbno,t1.tb_id
		,t1.outcomes
...
foreach($result as $value) { 
	?>	
	<input type="checkbox" value="<?=$value;?>" name="checkbox[]"/><?=$value;?>
   <?}?>
